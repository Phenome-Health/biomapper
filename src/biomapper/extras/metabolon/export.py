...
from biomapper.models import MappingResult, MappingSummary
...
def flatten_results(results: list[MappingResult]) -> list[dict[str, Any]]:
    ...
    Args:
        results: Output of :func:`biomapper.map_entities` or equivalent.
...
def results_to_dataframe(results: list[MappingResult]) -> Any:  # noqa: ANN401 — pandas is an optional extra; return type is pandas.DataFrame when installed
    ...
    Requires ``biomapper[metabolon]`` (pandas).

    Args:
        results: Mapping results from any biomapper mapping function.
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install with: pip install 'biomapper[metabolon]'"
        ) from exc
...
def save_results(
    ...
    Args:
        ...
        summary:   Optional :class:`~biomapper.models.MappingSummary` to embed
                   in the JSON output.
